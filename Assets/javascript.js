...
(() => {
  ...
  if (!root) {
    console.error("#" + mountId + " not found");
    return;
  }
...
function updatePrayerTimeUI(prayerTimes, location, methodName) {
  ...
  prayTimeDiv.innerHTML = `
        <div style="background: rgb(147 147 147 / 10%); backdrop-filter: blur(10px); border-radius: 15px; padding: 20px; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.2); color: white; text-align: center; max-width: 600px; margin: 0 auto;">
            ...
            <p style="margin-bottom: 5px; opacity: 0.9; color: var(--text);">স্থান: ${
              location || "লোড হচ্ছে..."
            }</p>
            <p style="margin-bottom: 15px; opacity: 0.9; color: var(--text);">পদ্ধতি: ${
              methodName || "ইসলামিক সোসাইটি"
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; margin-bottom: 20px;">
                ...
                <div style="background: var(--card); padding: 15px; border-radius: 10px; grid-column: span 1;">
                    ...
                    <div style="font-size: 1.1em; color: var(--text);">সময়: ${
                      nextPrayerInfo.nextPrayerTime
                    }</div>
                    <div style="font-size: 1.1em; color: var(--text);">বাকি: ${
                ...
                `
                    ...
                    <div class="jumaDiv" style="padding: 15px; border-radius: 10px; grid-column: span 2;">
                        ...
                        <div style="font-size: 1.2em; font-weight: 700; margin-bottom: 5px;">${
                          nextPrayerInfo.jummahDate
                        }</div>
                        <div style="font-size: 1.1em;">সময়: ${
                          nextPrayerInfo.jummahTime
                        }</div>
                        <div style="font-size: 1.1em;">বাকি: ${
                          nextPrayerInfo.isJummah
                            ? 0
                ...
                <div style="background: var(--card); padding: 12px; border-radius: 10px;">
                    <div style="font-weight: 600; margin-bottom: 5px; color: var(--text);">যোহর${
                      nextPrayerInfo.isJummah ? " (জুমআ)" : ""
                    }</div>
                    <div style="color: var(--text);">${formatTime(
                      prayerTimes.dhuhr
                    )}</div>
...
(async function () {
  ...
  container.innerHTML = `
    ...
    <div class="controls">
      <label class="TopTitt">- সূরা 
        <select id="surahSelect" style="padding: 8px; margin: 5px; border-radius: 5px; border: 1px solid #ccc;"></select>
      </label>
      <label class="TopTitt">- আয়াত (শুরু) 
        <select id="ayahFrom" style="padding: 8px; margin: 5px; border-radius: 5px; border: 1px solid #ccc;"></select>
      </label>
      <label class="TopTitt">- আয়াত (শেষ) 
        <select id="ayahTo" style="padding: 8px; margin: 5px; border-radius: 5px; border: 1px solid #ccc;"></select>
      </label>
      <label class="TopTitt">- কারী 
        <select id="reciterSelect" style="padding: 8px; margin: 5px; border-radius: 5px; border: 1px solid #ccc;"></select>
      </label>
      <label class="TopTitt">
        <input type="checkbox" id="continuousPlayback"> সব আয়াত একসাথে দেখুন
      </label>
      <button id="loadBtn" style="padding: 8px 15px; margin: 5px; border-radius: 5px; border: none; background: #4CAF50; color: white; cursor: pointer;">সার্চ করুন</button>
    </div>
    <div id="output"></div>
  `;
  ...
  // শেয়ার ফাংশন
const shareAyah = (arabicText, banglaText, surahNumber, ayahNumber) => {
  const siteUrl = window.location.href;
  const shareText = `সূরা ${surahNumber}, আয়াত ${ayahNumber}:\n\n${arabicText}\n\n${banglaText}\n\n- কুরআন মাজীদ\n\n${siteUrl}`;
  
  if (navigator.share) {
    navigator.share({
      title: 'কুরআন শরীফ',
      text: shareText
      // url: siteUrl  <-- এই লাইনটা বাদ দিন
    })
    .catch(error => {
      console.log('শেয়ারিং এ সমস্যা:', error);
  ...
  const copyToClipboard = (text) => {
    ...
    alert('আয়াতটি ক্লিপবোর্ডে কপি করা হয়েছে! আপনি এখন এটি শেয়ার করতে পারেন।');
  };
  ...
  loadBtn.addEventListener("click", async () => {
    ...
    const isContinuous = continuousPlayback.checked;
    ...
    try {
      ...
      if (isContinuous) {
        // একসাথে প্লে করার জন্য একটি মাত্র অডিও এলিমেন্ট
        const audioUrls = [];
        let arabicText = "";
        let banglaText = "";
        
        for (let i = from - 1; i < to; i++) {
          audioUrls.push(audioAyahs[i].audio);
          arabicText += arAyahs[i].text + " ";
          banglaText += (bnAyahs[i] ? bnAyahs[i].text : "") + " ";
        }
        
        html += `
          <div class="verse">
            <div class="ar">${arabicText}</div>
            <div class="bn">${banglaText}<div><i class="fa fa-share-alt share-btn" title="শেয়ার করুন" style="display: flex;margin-right: 20px;cursor: pointer;justify-content: flex-end;""></i></div></div>
            <audio controls id="continuous-audio"></audio>
          </div>
        `;
        
        output.innerHTML = html;
        
        // শেয়ার বাটনে ইভেন্ট লিস্টেনার যোগ
        const shareBtn = output.querySelector('.share-btn');
        shareBtn.addEventListener('click', () => {
          const surahName = surahs.find(s => s.number == surahNum).name;
          shareAyah(arabicText, banglaText, surahName, `${from}-${to}`);
        });
        
        // একসাথে অডিও প্লে করার জন্য
        const continuousAudio = document.getElementById("continuous-audio");
        
        // অডিও সোর্স সেট করা
        continuousAudio.src = audioUrls[0];
        
        // বর্তমান অডিও ইনডেক্স ট্র্যাক করা
        let currentAudioIndex = 0;
        
        continuousAudio.addEventListener("play", () => {
          stopCurrentAudio();
          currentlyPlayingAudio = continuousAudio;
        });
        
        continuousAudio.addEventListener("ended", () => {
          currentAudioIndex++;
          if (currentAudioIndex < audioUrls.length) {
            continuousAudio.src = audioUrls[currentAudioIndex];
        ...
        continuousAudio.addEventListener("pause", () => {
          if (currentlyPlayingAudio === continuousAudio) {
            currentlyPlayingAudio = null;
          }
        });
      } else {
        // আলাদা আলাদা আয়াত দেখানো
        for (let i = from - 1; i < to; i++) {
          const ar = arAyahs[i];
          const bn = bnAyahs[i];
          ...
          html += `
            ...
            ar.numberInSurah
              ...
              <div class="bn">${bn ? bn.text : "<em>বাংলা অনুবাদ নেই</em>"}<div><i class="fa fa-share-alt share-btn" title="শেয়ার করুন" data-index="${i}" style="display: flex;margin-right: 20px;cursor: pointer;justify-content: flex-end;""></i></div></div>
        ...
        // শেয়ার বাটনে ইভেন্ট লিস্টেনার যোগ
        const shareBtns = output.querySelectorAll('.share-btn');
        shareBtns.forEach(btn => {
          btn.addEventListener('click', (e) => {
            const index = parseInt(e.target.getAttribute('data-index'));
            const arabicText = arAyahs[index].text;
            const banglaText = bnAyahs[index] ? bnAyahs[index].text : '';
            const surahName = surahs.find(s => s.number == surahNum).name;
            const ayahNumber = arAyahs[index].numberInSurah;
            
            shareAyah(arabicText, banglaText, surahName, ayahNumber);
          });
        });
        ...
        audios.forEach((audio) => {
          ...
          audio.addEventListener("pause", () => {
            // Only clear reference if this is the currently playing audio
          ...
          audio.addEventListener("ended", () => {
            // Clear the currently playing audio when it ends
            if (currentlyPlayingAudio === audio) {
              currentlyPlayingAudio = null;
            }
            
            // Play the next audio if available
            const nextIdx = parseInt(audio.getAttribute("data-idx")) + 1;
            const nextAudio = output.querySelector(`audio[data-idx="${nextIdx}"]`);
            if (nextAudio) {
              nextAudio.play();
